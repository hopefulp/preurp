...
def main():
    ...
    parser.add_argument('-nt', '--ntraj', default=1, type=int, help='several trajectories')
    ...
    ### 20 % traj will be kept for test
        ...
        ntrain = int(args.ntraj * 0.8)
        ntest  = args.ntraj - ntrain
        for i in range(args.ntraj):
            print(f"running {i+1}/{args.ntraj}-th trajectory")
            if i < ntrain:
                name = "emtmd"
            else:
                name = 'test'
